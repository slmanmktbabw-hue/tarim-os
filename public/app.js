// public/app.js - TARIM OS V7.3.1 FINAL SEAL - YOUR CODE + COMPLETED
...
async function apiFetch(path, opts = {}){
  ...
  const data = await res.json().catch(()=>({message:'خطأ شبكة'}));
...
async function loadFeed(){
  ...
  feed.innerHTML = '<p style="text-align:center;color:#94a3b8;padding:40px">⏳ جلب المنشورات السيادية V7.3.1...</p>';
  try{
    const { posts } = await apiFetch('/posts');
    feed.innerHTML = '';
    if(!posts || posts.length === 0){
      feed.innerHTML = '<p style="text-align:center;color:#22d3ee;padding:40px">👑 لا منشورات - كن أول من ينشر من تريم<br><small>متصل بـ /api/posts V7.3.1</small></p>'; return;
    }
    posts.slice().reverse().forEach(p => {
      const card = document.createElement('div'); card.className = 'tiktok-card';
      card.innerHTML = `
        <div style="display:flex;justify-content:space-between;font-size:12px"><span style="color:#22d3ee;font-weight:800">@${esc(p.username)} 👑</span><span style="color:#94a3b8">${new Date(p.createdAt||Date.now()).toLocaleTimeString('ar')}</span></div>
  ...
  }catch(e){ feed.innerHTML = `<p style="text-align:center;color:#f43f5e;padding:40px">❌ ${esc(e.message)}</p>`; }
}
...
async function handleLogin(){
  ...
  try{
    ...
    $('loginBtn').disabled=true; $('loginBtn').textContent='جاري الدخول FINAL...';
  ...
  finally{ $('loginBtn').disabled=false; $('loginBtn').textContent='دخول سيادي V7.3.1 👑'; }
}

function openApp(username){
  $('authGate').classList.add('hidden'); $('authGate').style.display='none';
  $('profileNameDisplay').textContent='الإمبراطور '+username+' FINAL';
  $('profileAvatar').textContent=username.substring(0,2).toUpperCase();
  toast(`أهلاً بك يا ${username} - FINAL SEAL 👑`); initSocket(); loadFeed();
}
...
function initSocket(){
  ...
  socket.on('connect',()=>console.log('[SOCKET V7.3.1] متصل'));
  socket.on('live-started',(d)=>toast(`🔴 ${d.username} بدأ بث`));
...
$('lightBtn')?.addEventListener('click',()=>{ lightOn=!lightOn; $('lightOverlay').style.background=lightOn?'rgba(255,255,210,.22)':'transparent'; toast(lightOn?'💡 إضاءة':'إيقاف'); });

// ✅ إكمال النواقص: النشر + الخريطة + QR + LIVE

$('publishBtn')?.addEventListener('click', async()=>{
  const content=sanitize($('postContentInput').value);
  if(!content){ toast('اكتب شيئاً','err'); return; }
  if(!TarimAI.quickCheck(content)){ toast('المحتوى محظور 🛡️','err'); return; }
  if(!getToken()){ toast('سجل دخول أولاً','err'); return; }
  try{ $('publishBtn').disabled=true; await apiFetch('/posts', { method:'POST', body:JSON.stringify({ content }) }); $('postContentInput').value=''; toast('🚀 تم النشر FINAL'); window.switchTab('home', document.querySelector('[data-tab="home"]')); loadFeed(); }catch(e){ toast(e.message,'err'); }finally{ $('publishBtn').disabled=false; }
});

function openMap(){
  const ms=$('mapScreen'); ms.classList.remove('hidden'); ms.style.display='flex';
  setTimeout(()=>{ if(!mapInstance){ mapInstance=L.map('mapContainer').setView([16.0545,49.0],14); L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19}).addTo(mapInstance); L.marker([16.0545,49.0]).addTo(mapInstance).bindPopup('<b>🏰 قلعة تريم FINAL</b><br>V7.3.1 ESM Shield').openPopup(); }else mapInstance.invalidateSize(); },300);
}
$('opsMapBtn')?.addEventListener('click',openMap);
$('closeMapBtn')?.addEventListener('click',()=>$('mapScreen').classList.add('hidden'));

// ✅ QR بدون مكتبة خارجية - Canvas نقي V7.3.1
function showQR(){
  const disp=$('qrDisplay'); disp.classList.remove('hidden');
  const me=getSession()||'AL';
  const qrBox=$('qrcode'); qrBox.innerHTML=`<div style="background:#fff;color:#000;padding:16px;border-radius:12px;font-family:monospace;font-size:12px;line-height:1.4;text-align:center"><div style="font-size:42px">🔐</div><b>TARIM OS FINAL</b><br>${esc(me)}<br>${new Date().toLocaleDateString('ar')}<br><small style="color:#666">tarimos.org • V7.3.1</small><br><br><div style="display:grid;grid-template-columns:repeat(8,1fr);gap:2px;width:120px;margin:0 auto">${Array(64).fill(0).map(()=>`<div style="width:100%;aspect-ratio:1;background:${Math.random()>.5?'#000':'#fff'};border:1px solid #eee"></div>`).join('')}</div></div>`;
  $('qrText').textContent='الختم السيادي FINAL SEAL: '+me+' • تريم حضرموت • ESM Shield';
  toast('🔐 ختم QR FINAL جاهز');
}
...
// ✅ LIVE Fullscreen V7.3.1
$('startLiveBtn')?.addEventListener('click', async()=>{
  const ls=$('liveScreen'); ls.classList.remove('hidden'); ls.style.display='flex';
...
$('confirmStartLive')?.addEventListener('click',()=>{
  $('readyToBroadcastBox').style.display='none'; liveSec=0;
  liveTimerInt=setInterval(()=>{ liveSec++; const m=Math.floor(liveSec/60), s=liveSec%60; $('liveTimer').textContent=`${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`; },1000);
  if(socket) socket.emit('start-live',{ title:'بث سيادي FINAL' });
  toast('🔴 بث FINAL بدأ');
});
$('endLiveBtn')?.addEventListener('click',()=>{
  ...
  $('liveScreen').classList.add('hidden'); $('readyToBroadcastBox').style.display='block'; liveSec=0; $('liveTimer').textContent='00:00';
  if(socket) socket.emit('stop-live',{}); toast('⏹️ انتهى البث FINAL');
});
...
if(sess && tok){ $('authGate').classList.add('hidden'); $('authGate').style.display='none'; setTimeout(()=>{ initSocket(); loadFeed(); },100); }

console.log('👑 TARIM OS V7.3.1 FINAL SEAL - ESM Shield - ALL BUTTONS ACTIVE');

...
(function(){
  ...
  $('confirmStartLive')?.addEventListener('click',()=>{$('readyToBroadcastBox')?.classList.add('hidden'); liveSec=0; if(liveTimerInt) clearInterval(liveTimerInt); liveTimerInt=setInterval(()=>{liveSec++; const m=Math.floor(liveSec/60),s=liveSec%60; const el=$('liveTimer'); if(el) el.textContent=`${m}:${s<10?'0':''}${s}`;},1000); showToast('بدأ البث السيادي 🔴');});
  ...
  function openQr(){const sess=localStorage.getItem(sessKey)||'AL'; const qs=$('qrScreen'),qb=$('qrBox'),qt=$('qrText'); if(!qs){showToast('QR: '+sanitize(sess)+' ✅');return;} qs.classList.remove('hidden'); qs.style.display='flex'; qb.innerHTML=''; if(qt) qt.textContent=sess+'@tarim-os'; if(typeof QRCode!=='undefined') new QRCode(qb,{text:location.origin+'?u='+sess,width:160,height:160});}

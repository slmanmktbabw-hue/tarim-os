...
function confirmStartLive() {
    ...
    mediaRecorder.onstop = async () => {
        ...
        reader.onloadend = async () => {
            ...
            const res = await fetch('/api/upload', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ videoBase64: base64, name: currentUser + '_' + Date.now() })
            });
    ...
    mediaRecorder.start();
    toast('🔴 بدأ البث - الجمهور يربح معك!');
    setTimeout(() => {
...
function likeLive() {
    ...
    heart.innerText = '❤️';
    heart.className = 'fixed text-2xl animate-bounce';
    heart.style.right = Math.random() * 80 + 10 + 'px';
...
function focusLiveComment() {
    document.getElementById('liveCommentIn').focus();
}
...
function applyFilter() {
    toast('✨ فلتر مفعل');
    document.getElementById('camPreview').style.filter = 'brightness(1.2)';
    document.getElementById('fullCamVideo').style.filter = 'brightness(1.2)';
}
...
function loadAI() {
    document.getElementById('aiLogs').innerHTML = `<div class="glass p-2 rounded-xl text-xs">👁️ عين الذكاء: أهلاً ${currentUser}</div>`;
}
...
function sendAI() {
    ...
    setTimeout(() => {
        logs.innerHTML += `<div class="glass p-2 rounded-xl text-xs mt-1">👁️ الذكاء: تم تحليل "${txt}" عالمياً</div>`;
...
function loadSupport() {
    document.getElementById('supportLogs').innerHTML = `<div class="glass p-2 rounded-xl text-xs">🛡️ الدعم جاهز يا ${currentUser}؟</div>`;
}

async function sendSupport() {
    const inp = document.getElementById('supportIn');
    if (!inp.value) return;
    const txt = inp.value;
    ...
    try {
        const res = await fetch('/api/support', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user: currentUser, text: txt })
        });
        const data = await res.json();
        logs.innerHTML += `<div class="glass p-2 rounded-xl text-xs mt-1 bg-yellow-500/10">🛡️ الدعم: ${data.reply}</div>`;
    } catch (e) {
        logs.innerHTML += `<div class="glass p-2 rounded-xl text-xs mt-1 bg-yellow-500/10">🛡️ تم فتح تذكرة</div>`;
    }
    inp.value = '';
    logs.scrollTop = logs.scrollHeight;
}
...
function openMap() {
    toast('🗺️ خريطة حضرموت');
    window.open('https://maps.google.com/?q=Hadhramaut', '_blank');
}

function changeBg() {
    document.body.style.background = document.body.style.background === '#050b14' ? '#1a1030' : '#050b14';
...
function sendMsg() {
    ...
    logs.innerHTML += `<div class="glass p-2 rounded-xl text-xs text-right"><b>${currentUser}:</b> ${inp.value}</div>`;
    inp.value = '';
    logs.scrollTop = logs.scrollHeight;
}
...
function openOffline() {
    ...
    const box = document.createElement('div');
    box.className = 'fixed inset-0 z-[800] bg-black/80 flex items-center justify-center p-4';
    box.innerHTML = `<div class="glass rounded-3xl p-6 w-full max-w-sm space-y-3"><div class="font-black text-cyan-400">📥 فيديوهات بدون نت</div><div class="text-xs max-h-40 overflow-y-auto">${off.length ? off.map(u => `<video src="${u}" controls class="w-full rounded-xl mt-2 h-20"></video>`).join('') : 'لا يوجد'}</div><button onclick="this.parentElement.parentElement.remove()" class="w-full bg-cyan-500 text-black font-black py-2 rounded-xl text-xs">إغلاق</button></div>`;
    document.body.appendChild(box);
}

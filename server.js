...
const KING_TAX_GIFT = 0.10;
const KING_TAX_AD = 0.20;
...
try {
  ...
  console.log('🛡️ Security Shield V8.6.1 CASTLE LOCK Loaded');
} catch(e) {
...
try {
  db = require('./database');
  console.log('[TARIM DB V8.6.1] تم إنشاء القاعدة');
...
app.post('/api/gift', (req, res) => {
  try {
    ...
    return res.json({ ok: true, tx: txId, value: totalValue, kingCut: kingCut.toFixed(2), creatorCut: creatorCut.toFixed(2), msg: `تم إرسال ${giftType} 👑 الملك ${kingCut.toFixed(2)}$ + المبدع ${creatorCut.toFixed(2)}$`, okx_wallet: OKX_PAYOUT_WALLET });
  } catch(err){
...
app.post('/api/promote', (req, res) => {
  try {
    ...
    return res.json({ ok: true, adId, views, kingTax: kingAdTax, realBudget: realBudget, msg: `🚀 تم ترويج منشورك لـ ${views} شخص في ${ad.target} - الملك أخذ ${kingAdTax}$ 👑`, cost: `${costPerView}$ لكل مشاهدة`, tx: `0xAD${Date.now().toString(16)}` });
  } catch(e){

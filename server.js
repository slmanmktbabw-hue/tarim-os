...
// نظام الذاكرة والمهام السيادية
let memoryTasks = [
    { id: 1, title: 'بث مباشر سيادي ومشفر (8 دقائق)', description: 'سيرفرات أسطورية • إرسال واستقبال هدايا • فلاتر بصرية', status: 'بث نشط', completed: true },
    ...
    { id: 3, title: 'خريطة حضرموت وتريم بدون نت (Offline)', description: 'تسجيل صوتي AES • مشاركة سيادية مهام خلال 5 دقائق', status: 'ميداني', completed: true },
    { id: 4, title: 'إصدار الختم الميداني المشفر + QR', description: 'توليد منشورات درع الفصيل • عين الذكاء Tesseract', status: 'جاهز', completed: false }
];
...
// API التحقق السيادي للـ CEO وبصمة الأمان
...
// WebSocket للعدادات والبث المباشر
io.on('connection', (socket) => {
    socket.on('update_live_metrics', (data) => io.emit('broadcast_metrics', data));
});

// توجيه كل الطلبات إلى الواجهة الرئيسية index.html
app.get('*', (req, res) => {
